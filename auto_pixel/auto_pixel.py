...
def pause():
    ...
    dbfile.close()
    logging.debug('pause thread: pause status just after start of pause(): %s'
                  % db['pause'])
    ...
    # have two separate [in/out/err) streams for the pause thread's
    # inf loop and the main thread
    while True:
        ...
        if option.lower() == 'resume' or option.lower() == 'r':
            ...
            dbfile = open('configure', 'rb')
            db = pickle.load(dbfile)
            logging.debug("pause thread: initial pause status: "+db['pause'])
            dbfile.close()
...
def get_confs(outer=True):
    ...
    while not target_obtained:
        print('Either take the cursor to the target color in color palette and'
              ' press Enter')
    ...
    # return window_edge,top_edge,bottom_edge,freeze_time,target_color,
    #        exception_colors
    if outer:
...
if os.path.isfile('configure'):
    ...
    if user.lower() == 'y':
        ...
        print_intro()
        db = get_confs()
        logging.debug('at main thread: value obtained from user -- configure'
                      ' file overwritten')
else:
    logging.debug('printing intro and getting values from user')
    print_intro()
    db = get_confs()
    logging.debug('at main thread: value obtained from user -- new configure'
                  ' file created')
...
if not lines:
    print_intro()
    window_edge,top_edge,bottom_edge,freeze_time,target_color,
            exception_colors=get_confs()
...
while True:
    ...
    if loop_cycle % per_shot:
        ...
        logging.debug('loop_cycle%per_shot gave true, opening file to'
                      ' write screenshot')
        fname = 'shot-%s-%d.png' % (datetime.datetime.now().strftime('%y%m%d%H%M%S'),
                                    loop_cycle)
        try:
            fname = os.path.join(os.getcwd(), 'notebook.logs.screenshots', fname)
        except Exception as e:
            print('Could not open the file, make sure that the program\'s'
        ...
        logging.debug('writing scrn.crop() object (of type: %s) to file' %
                      str(type(im_data)))
        im_data.save(fname)
    ...
    if im_element is not None:
        ...
        if clickpoint[0] > window_edge:
            ...
            logging.info('Point clicked at coord: %s , sleeping for 4 seconds' %
                         str(clickpoint))
            time.sleep(4)
        ...
        if clickpoint[0] > window_edge:
            ...
            logging.info('Point clicked at coord: %s , sleeping for 4 seconds' %
                         str(clickpoint))
            time.sleep(4)
    elif pyautogui.locateOnScreen('captcha.png') is not None:
        ...
        if clickpoint[0] > window_edge:
            ...
            if captcha_fail > 3 and not captcha_sent:
                logging.info('Sending captcha to C&C, captcha_fail = %d' %
                             captcha_fail)
                parted_scrn = scrn.crop((window_edge, 0, width, height))
                ...
                logging.debug('captcha_fail = %d, user being informed of captcha' %
                              captcha_fail)
                print('CAPTCHA verification appeared')
                time.sleep(60)
                captcha_fail += 1
                continue
            logging.debug('clicking coord: (%d, %d) to bring browser in focus' %
                          (window_edge+10, top_edge+10))
    ...
    if pyautogui.locateOnScreen('skipad.png'):
        ...
        if clickpoint[0] > window_edge:
            logging.debug('skip ad is in operated screen area, clicking "skip ad"'
                          ' at %s' % str(clickpoint))
    ...
    for x in range(window_edge, width, random.randint(5, 10)):
        # l ogging.debug('inside x nested loop: x=%d'%x)
        scrn = pyautogui.screenshot()
        for y in range(top_edge, bottom_edge, random.randint(5, 10)):
            ...
            if xyloop_count % 200:
                ...
                logging.debug('inside y nested loop: (x,y)=(%d, %d), RGB=%s ,'
                              ' Color: %s' %( x, y, str(pixel_color), color_name))
            ...
            if pixel_color in clickables:
                logging.debug('pixel of clickable color found: (x,y)= (%d, %d)...'
                              % (x, y))
                logging.debug('...  RGB: %s, color: %s' % (str(pixel_color),
                              color_meanings[pixel_color]))
                dbfile = open('configure', 'rb')
                # pause_line=cfile.readlines()[0].split('=')[1][:-1]
                db = pickle.load(dbfile)
                dbfile.close()
                logging.debug('(inside clickable pixel procedure): pause value: %s'
                              % db['pause'])
                pause_value = db['pause']
                if pause_value == 'QUIT':
                    logging.debug('(inside clickable pixel procedure): pause value'
                                  ' is "QUIT", calling sys.exit()')
                    sys.exit()
                ...
                fname = 'pixel_shot-%s-%d-before.png' %\
                        (datetime.datetime.now().strftime('%y%m%d%H%M%S'), loop_cycle)
                try:
                ...
                logging.info('Point clicked at (%d, %d), taking screenshot again'
                             ' after clicking' % (x+4, y+4))
                # exactly same name as that of before clicking shot,
                # except append "after"
                fname = fname[:-10]
                fname += 'after.png'
    ...
    if not xybreak:
        ...
        fname = 'no_clicks_shots-%s-%d.png' % (datetime.datetime.now().
                                               strftime('%y%m%d%H%M%S'), loop_cycle)
